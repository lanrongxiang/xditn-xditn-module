...
namespace XditnModule\Support\Zip;
...
class Zipper
{
    ...
    public function setExcludeDirs(array $dirs): static
    {
    ...
    private function addDir($pathToDir): void
    {
        ...
        foreach ($this->file->files($pathToDir) as $file) {
            $this->addFile($pathToDir.'/'.$file->getBasename());
        }

        // Now let's visit the subdirectories and add them, too.
        foreach ($this->file->directories($pathToDir) as $dir) {
            if (in_array(str_replace('/', DIRECTORY_SEPARATOR, $dir), $this->excludeDirs)) {
                continue;
            }
