...
abstract class Installer
{
    ...
    protected function migrate(): void
    {
        if (app()->runningInConsole()) {
            $migrationStr = sprintf('xditn:module:migrate %s', $this->info()['name']);
            command($migrationStr);
        } else {
            Artisan::call('xditn:module:migrate', [
    ...
    {
        if (app()->runningInConsole()) {
            $seedStr = sprintf('xditn:module:db:seed %s', $this->info()['name']);
            command($seedStr);
        } else {
            Artisan::call('xditn:module:db:seed', [
                'module' => $this->info()['name'],
            ]);
        }
    }
    ...
    public function install(): void
    {
        ...
        if (method_exists($this, 'dependencies')) {
            ...
            $enabledModules = Module::getEnabled()->pluck('name')->merge(Collection::make(config('xditn.module.default')));

            foreach ($dependencies as $dependency) {
                if ($enabledModules->contains($dependency)) {
                    continue;
                }
                $moduleInstaller = XditnModule::getModuleInstaller($dependency);
